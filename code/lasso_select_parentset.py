...
class LassoParentSelection():
    ...
    def __call__(self):
        ...
        for Yunit_id in units:
            ...
            area_units, subset_indices = {a:[] for a in AREAS} , {a:{'+ve': [], '-ve': [], 'all':[]} for a in AREAS }
            ...
            if DI_Y_X[self.stimulus_id] > 0:
                ...
                area_models = {}
                area_HY_X = {a: {} for a in AREAS}
                area_HY_X_all = {a: None for a in AREAS} # with all data combined 
...
def main(config, area_filter, calculate_rates):
    ...
    # for dataset, prefiltered_dataset in zip(datasets, prefiltered_datasets):
        ...
        for stimulus_id in stimuli:
            LassoParentSelection(config, dataset, data, stimulus_id=int(stimulus_id), area_filter=area_filter, calculate_rates=calculate_rates)()
...
if __name__ == "__main__":
    ...
    main(conf, args.area_filter, args.rates)
