...
def get_adjacency(file_dir, stimulus_id, rates_path=None, min_rate=0., percentile=95, use_ate=False):
    '''
...
#     import ipdb; ipdb.set_trace()
    ...
    for f in files:
        ...
        if use_ate:
            ...
            if di > 0: 
                result = {}
                areas_ = list(set([x.split('_')[-2] for x in data['selected_parents']]))
    ...
    #             contributions = {a:HY - (data['area_models'][a]['HY_X'][stimulus_id] + data['HY_X'][stimulus_id])/2 for a in  areas_ }
